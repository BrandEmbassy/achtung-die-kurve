  ...
  interfere(point, additionalDistance, headException = true) {
    for (let j = 0; j < this.segments.length; j++) {
      ...
      for (let i = 0; i < segment.pointsHistory.length; i++) {
        ...
        if (
          distance > segment.weight / 2 &&
          distance < additionalDistance + segment.weight / 2
        ) {
          // console.log(distance, additionalDistance, this.weight / 2);
          const rest = [].concat.apply(
            [],
            this.segments
              .slice(j + 1, this.segments.length)
              .map((segm) => segm.pointsHistory)
          )
          if (!headException) return true
          return segment.pointsHistory
            .slice(i, segment.pointsHistory.length)
            .concat(rest)
            .some((p) => {
              return (
                Math.sqrt((point.x - p.x) ** 2 + (point.y - p.y) ** 2) >
                additionalDistance + segment.weight / 2
              )
            })
        }
      }
